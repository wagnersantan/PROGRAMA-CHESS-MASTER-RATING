...
def cadastrar_enxadrista():
    ...
    if collection.find_one({'nome': nome}):
        messagebox.showerror("Erro", "Enxadrista com esse nome já cadastrado!")
        return
...
# Função para excluir o enxadrista
def excluir_enxadrista():
    # Obter o item selecionado na tabela
    selecionado = tree.selection()
    if not selecionado:
        messagebox.showerror("Erro", "Nenhum enxadrista selecionado!")
        return
    
    # Obter o nome do enxadrista selecionado
    nome = tree.item(selecionado, "values")[1]  # O nome está na segunda coluna
    
    # Confirmar exclusão
    confirmacao = messagebox.askyesno("Confirmar", f"Tem certeza que deseja excluir {nome}?")
    if not confirmacao:
        return
    ...
    atualizar_tabela()
    messagebox.showinfo("Sucesso", f"Enxadrista {nome} excluído com sucesso!")
...
btn_excluir = ttk.Button(frame_botoes, text="Excluir Enxadrista", command=excluir_enxadrista)
btn_excluir.pack(side="left", padx=5)

# Atualizar a tabela ao iniciar o programa
atualizar_tabela()
